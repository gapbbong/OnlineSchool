      ...
      <div className="flex-1 max-w-4xl mx-auto w-full p-6 space-y-6">
        ...
        {showForm && (
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-3">
            ...
            <button onClick={handleCreate} disabled={busy || !form.work_title} className="flex items-center px-4 py-2 bg-sky-600 hover:bg-sky-700 disabled:opacity-70 disabled:cursor-not-allowed text-white text-xs font-bold rounded-lg transition">
              {busy && <ButtonSpinner />}
              {busy ? "처리 중..." : "등록"}
            </button>
          </div>
        ...
        {items === null && <PanelLoading label="인수인계 목록을 불러오는 중입니다..." />}
            ...
            <div key={h.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 space-y-2">
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 px-3 py-1 bg-amber-600 hover:bg-amber-700 disabled:opacity-70 disabled:cursor-not-allowed text-white text-[11px] font-bold rounded transition"
                >
                  {busy ? <ButtonSpinner /> : <ArrowRightLeft className="w-3 h-3" />} 인수인계
                </button>
              </div>
            </div>
          ))}
